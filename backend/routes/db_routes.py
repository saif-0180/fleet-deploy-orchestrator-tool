...
def get_db_connections():
    try:
        ...
        return jsonify([
            {
                "db_connection": "app_db",
                "hostname": "10.172.145.204", 
                "port": "5400", 
                "db_name": "ocstc1t"
            },
            {
                "db_connection": "asm_db",
                "hostname": "10.172.145.204", 
                "port": "5400", 
                "db_name": "passpg"
            }
...
def get_db_users():
    try:
        ...
        if os.path.exists(inventory_path):
            with open(inventory_path, 'r') as f:
                inventory = json.load(f)
                return jsonify(inventory.get('db_users', ["xpidbo1refc", "xpidbo1cfg", "xpidbo1app"]))
        
        # Fallback to default values if inventory file not found
        return jsonify(["xpidbo1refc", "xpidbo1cfg", "xpidbo1app"])
    except Exception as e:
