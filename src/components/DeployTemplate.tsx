...
// Status Icon Component for LogDisplay
const StatusIcon: React.FC<{
  ...
  const getStatusIcon = () => {
    switch (status) {
      ...
      case 'running':
      case 'loading':
        return <Loader2 className={`h-5 w-5 animate-spin text-blue-500 ${className}`} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          {/* Updated LogDisplay with custom title that includes status icon */}
          <Card className="bg-[#1a2b42]">
            <CardHeader className="pb-3">
              <CardTitle className="text-[#EEEEEE] text-lg flex items-center justify-between">
                <span>
                  {`Template Deployment Logs${currentDeploymentId ? ` - ${currentDeploymentId}` : ''}`}
                </span>
                <StatusIcon status={logStatus} />
              </CardTitle>
            </CardHeader>
            <CardContent>
...
//       }
//       if (line.includes("=== Executing")) {
//         return "Executed deployment steps";
//       }
//     }
